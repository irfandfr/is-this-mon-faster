...
const PkmnAdvancedSelector = lazy(
  () => import("./subComponent/PkmnAdvancedSelector")
);
const PkmnSimpleSelector = lazy(
  () => import("./subComponent/PkmnSimpleSelector")
);
...
const CalculatorUI = ({ advanced }: CalculatorProp) => {
  ...
  const [stateTrickRoom, setTRstate] = useState({
    trick_room: {
      icon: <TrickRoomIcon />,
      title: "Trick Room",
      id: "trick_room",
  ...
  const GUIDE_DATA = [
    {
      ...
      title: "Active Ability",
      description:
        "Ability that doubles Speed under certain conditions such as weathers (Swift Swim), consuming item(Unburden), etc. (x2)",
    },
    {
      icon: <TailwindIcon />,
      title: "Tailwind",
      description: "A move that doubles the user’s and their allies Speed (x2)",
    },
    {
      icon: <ParalyzeIcon />,
      title: "Paralyze",
      description: "A status effect that reduces Speed by 50%(x0.5)",
    },
    {
      icon: <ChoiceScarfIcon />,
      title: "Choice Scarf",
      description: "A held item that increases the user Speed by 50%(x1.5)",
    },
    {
      icon: <IronBallIcon />,
      title: "Iron Ball",
      description: "A held item that decreases the user’s Speed by 50%(x0.5)",
    },
    {
      icon: <TrickRoomIcon />,
      title: "Trick Room",
      description:
        "A move that affects the field, the move makes slower Pokemon go first.",
    },
  ];
  ...
  function getTrValue() {
    return Object.entries(stateTrickRoom).map(
      (value: [string, SelectionProp]) => {
  ...
  function extractData(
    ...
    Object.entries(mods).forEach((value: [string, SelectionProp]) => {
      if (value[1].check) {
        pStats.push(
          modifiersAbbreviator(value[0] as keyof typeof InitialStateKey)
        );
      }
    });
    return pStats;
  ...
  function toResultPage(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    setTimeout(() => {
      //wait incase for debounce function
      ...
      let url = `/${advanced ? "adv" : "smp"}result?p1=${p1data.join(
        "_"
      )}&p2=${p2data.join("_")}&tr=${tr}`;
      navigate(url);
    }, 800);
  }

  useEffect(() => {
    document.title = `isMonFaster | ${
      location.pathname === "/calc" ? "Simple Compare" : "Advanced Compare"
    }`;
  }, [location]);
  ...
  return (
    <MainView className={style.calcUIContainer}>
      ...
      <div className={style.pageHeader}>
        ...
        <button className={style.faqButton} onClick={() => setModal(true)}>
          ?
        </button>
      </div>
      {advanced ? (
        <Suspense fallback={<LoadingPage errorText="" />}>
          <PkmnAdvancedSelector
            setP1Value={setP1Value}
            setP2Value={setP2Value}
            dispatch1={dispatch1}
            dispatch2={dispatch2}
            stateP1={stateP1}
        ...
        <Suspense fallback={<LoadingPage errorText="" />}>
          <PkmnSimpleSelector
            sprite1={p1Stat.imgLink}
            sprite2={p2Stat.imgLink}
            setP1Stat={setPkmn1Stat}
            setP2Stat={setPkmn2Stat}
            dispatch1={dispatch1}
            dispatch2={dispatch2}
            stateP1={stateP1}
            selectGroupState={selectGroupState}
      ...
      <div className={style.pContainer}>
        ...
        <InputGroup
          groupList={stateTrickRoom}
          onClick={trToggle}
          disabled={selectGroupState.trstate}
        />
      </div>
      <Button
        icon={<RefreshIcon className={style.refreshIcon} />}
        onClick={toResultPage}
        disabled={advanced ? false : !(!!p1Stat.name && !!p2Stat.name)}
        text="Analyze"
        type="primary"
        style={{ marginTop: "30px" }}
      />
      {advanced ? (
        <a className={style.link} href="/calc">
          Switch to Simple
        </a>
      ) : (
        <a className={style.link} href="/advanced">
          Switch to Advanced
        </a>
      )}
    </MainView>
